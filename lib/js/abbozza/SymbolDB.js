...
SymbolDB.prototype.getChild = function(name) {
	...
	for ( var i = 0; i < this.symbols.length; i++) {
		...
		if ( (symbol[0] == name) && (symbol[3] == this.FUN_SYMBOL) ) {
			return symbol[3];
		}
	}
	return null;
...
SymbolDB.prototype.toString = function() {
	...
	for ( var i = 0; i < this.symbols.length; i++ ) {
		...
		if ( entry[3] != Abbozza.FUN_SYMBOL ) {
			result = result + _(entry[1]) + " " + entry[0] + Abbozza.lenAsString(entry[2]);
			if ( entry[3] == Abbozza.PAR_SYMBOL ) result = result + " (Parameter)";
			result = result + "\n";
		}
	}
...
SymbolDB.prototype.getVarTypedMenu = function(type,suffix) {
	...
	for ( var i = 0; i < this.symbols.length; i++ ) {
		var entry = this.symbols[i];
		if ( (entry[3] != Abbozza.FUN_SYMBOL) && (entry[1] == type) && (entry[2] == null) ) {
			result = entry[0]; // + Abbozza.lenAsEmptyString(entry[2]);
			menu.push([result + " " + suffix,result]);
		}
	}
...
SymbolDB.prototype.getFuncMenu = function() {
	...
	for ( var i = 0; i < this.symbols.length; i++ ) {
            var entry = this.symbols[i];
            if ( (entry[3] == Abbozza.FUN_SYMBOL) && (entry[0] != 'setup') && ( entry[0]!='loop')) {
                result = entry[0]; // + Abbozza.lenAsEmptyString(entry[2]);
...
SymbolDB.prototype.toCode = function(prefix) {
	    var code = "";
 		var variables = this.getVariables(true);
 		for ( var i = 0; i < variables.length; i++ ) {
 			var entry = variables[i];
 			code = code + prefix + keyword(entry[1]) + " " + entry[0] + Abbozza.lenAsString(entry[2]) + ";";
 			if (( entry[4] != "") && (entry[4] != null)) code = code + "\t// " + entry[4].replace(/\n/g," ");
			code = code + "\n";
 		}
 		return code;
}
