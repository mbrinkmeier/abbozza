...
AbbozzaGenerator.prototype.typeList = function() {
	return [[_("VOIDT"),"VOID"],[_("NUMBER"),"NUMBER"],[_("STRING"),"STRING"],[_("DECIMAL"),"DECIMAL"],[_("BOOLEAN"),"BOOLEAN"]];
}
 ...
 for ( var i = 0; i<blocks.length; i++) {
 	...
 	if ( block.type == "devices" ) {
 		...
 		blocks[i] = temp;
                ...
                blocks[1] = block;
                blocks[i] = temp;
/* 	} else if ( block.type == "func_setup" ) {
 		temp = blocks[1];		
 		blocks[1] = block;
 		blocks[i] = temp;
 	} else if ( block.type == "func_loop" ) {
 		temp = blocks[2];		
 		blocks[2] = block;
 		blocks[i] = temp;                   */
...
AbbozzaGenerator.prototype.topBlockToCode = function(block) {
	var code = block.generateCode(this);
...
AbbozzaGenerator.prototype.blockToCode = function(block) {
	...
	/*
	if ( code == null ) {
		ErrorMgr.addError(block,_("err.EMPTYCODE"));
		return null;
	}
	*/
...
AbbozzaGenerator.prototype.valueToCodeUnchecked = function(block,name,defaultVal) {
	...
	var code = target.generateCode(this);
...
AbbozzaGenerator.prototype.addLibrary = function(line) {
