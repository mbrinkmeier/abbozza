...
TaskWindow.init = function () {
    ...
    TaskWindow.overlay_.appendChild(TaskWindow.editor_);
...
TaskWindow.showEditor = function () {
    TaskWindow.editor_.style.display = "block";
    var width = TaskWindow.page_.offsetWidth - 10;
    TaskWindow.editor_.style.width = width + "px";
    var height = TaskWindow.page_.offsetHeight - 10;
    TaskWindow.editor_.style.height = height + "px";
...
TaskWindow.setContent = function(html, reset) {
    ...
    if ((TaskWindow.currentPage_ < TaskWindow.pages_.length) && !reset) {
        TaskWindow.setPage_(TaskWindow.currentPage_);
    } else {
 ...
 */
TaskWindow.setPage_ = function (page) {
...
TaskWindow.setSize = function(width, height) {
    ...
    if ( height > window.innerHeight) {
        ...
        console.log(TaskWindow.overlay_.offsetTop);
 ...
 * 
 * @param {type} event The evnet causing the handlers activation.
 */
...
TaskWindow.editorChange_ = function(event) {
   TaskWindow.setContent(TaskWindow.editor_.value, false);    
}
