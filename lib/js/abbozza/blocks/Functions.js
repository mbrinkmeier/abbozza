...
Abbozza.FuncDecl = {
    ...
    generateCode: function (generator) {
        ...
        if (parameters.length > 0) {
            entry = parameters[0];
            pars = " " + generator.symbolToCode(entry); // keyword(entry[1]) + " " + entry[0] + Abbozza.lenAsString(entry[2]);
            ...
            for (var i = 1; i < parameters.length; i++) {
                entry = parameters[i];
                pars = pars + ",\n" + spaces + "  " + keyword(entry[1]) + " " + entry[0] + Abbozza.lenAsString(entry[2]);
                if (entry[4] != null && entry[4] != "") {
                    pars = pars + "\t// " + entry[4].replace(/\n/g, " ");
                }
            }
    ...
    updateLook: function () {
        ...
        for (var i = 0; i < variables.length; i++) {
            ...
            if (entry[3] == Abbozza.VAR_SYMBOL) {
                ...
                no++;
            } else if (entry[3] == Abbozza.PAR_SYMBOL) {
                params = params + ", " + entry[1] + " " + entry[0];
            }
        }

        params = params.substring(2);
        // this.getField_("PARAMS").setText(params);
    },
...
Abbozza.FunctionCall = {
    ...
    init: function () {
        ...
        this.appendDummyInput("INP")
                ...
                .appendField(new FunctionDropdown(this, function (entry) {
                    thisBlock.refit(entry);
                }), "NAME")
                .appendField(this.callLabel2);
        // this.appendDummyInput("PAR0").appendField("() " + __("func.CALL", 1));
    ...
    refit: function (name) {
        ...
        if (parameters.length == 0) {
            this.setInputsInline(true);
            // inp = this.appendDummyInput("PAR0").appendField("() " + __(localeEntry, 1));
        } else {
            ...
            for (no = 0; no < parameters.length; no++) {
                ...
                inp.appendField(parameters[no][0]);
    ...
    domToMutation: function (xmlElement) {
        ...
        if (parameters[0] == null) {
            ...
            while (parameters[no]) {
                this.setInputsInline(false);
                inp = this.appendValueInput("PAR" + no).setCheck(parameters[no].getAttribute("type"));
                if (no == 0) {
                    // inp.appendField("(")
                } else {
                    // inp.appendField(", ")
...
Abbozza.FunctionDeclControl = {
    init: function () {
        ...
        this.appendDummyInput()
                ...
                .appendField(new FieldNameInput("default", Abbozza.globalSymbols, Abbozza.FUN_SYMBOL), "NAME");
        this.appendDummyInput()
...
Abbozza.FunctionReturn = {
    ...
    generateCode: function (generator) {
        var result = generator.valueToCode(this, "VALUE");
