...
/**
 * @fileoverview Some utility operations for abbozza!
 * 
...
Abbozza.composeSymbolsAtInput = function(topBlock,funcBlock, parameter) {
	...
	var no = 0;
	while (block) {
	    if (block.getInput("DIM")) { block.getInput("DIM").type = inputType; }
		...
		for (var i = 0; i < variables.length ; i++ ) {
			...
			case "NUMBER":
                                if ( (entry[2] == null) && (Configuration.getParameter("option.noArrays") == "true")) {
                                    block = workspace.newBlock("devices_num_noconn"); // Blockly.Block.obtain(workspace,"devices_num_noconn");                                    
                                } else {
                                    block = workspace.newBlock("devices_num"); // Blockly.Block.obtain(workspace,"devices_num");
                                }
				block.getField("NAME").setText(entry[0]);
				break;
			case "STRING":
                                if ( (entry[2] == null) && (Configuration.getParameter("option.noArrays") == "true")) {
                                    block = workspace.newBlock("devices_string_noconn"); // Blockly.Block.obtain(workspace,"devices_string_noconn");                                    
                                } else {
                                    block = workspace.newBlock("devices_string"); // Blockly.Block.obtain(workspace,"devices_string");
                                }
				block.getField("NAME").setText(entry[0]);
				break;
			case "DECIMAL":
                                if ( (entry[2] == null) && (Configuration.getParameter("option.noArrays") == "true")) {
                                    block = workspace.newBlock("devices_decimal_noconn"); // Blockly.Block.obtain(workspace,"devices_decimal_noconn");                                    
                                } else {
                                    block = workspace.newBlock("devices_decimal"); // Blockly.Block.obtain(workspace,"devices_decimal");
                                }
				block.getField("NAME").setText(entry[0]);
				break;
			case "BOOLEAN":
                                if ( (entry[2] == null) && (Configuration.getParameter("option.noArrays") == "true")) {
                                    block = workspace.newBlock("devices_boolean_noconn"); // Blockly.Block.obtain(workspace,"devices_boolean_noconn");                                    
                                } else {
                                    block = workspace.newBlock("devices_boolean"); // Blockly.Block.obtain(workspace,"devices_boolean");
                                }
				block.getField("NAME").setText(entry[0]);
			}
			if ( block ) { 
