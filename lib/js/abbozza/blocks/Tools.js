...
Abbozza.decomposeSymbols = function (workspace, funcBlock, title, parameters) {
		...
		if ( parameters == false) {
			topBlock =  Blockly.Block.obtain(workspace,"devices_control");
		} else {
			topBlock =  Blockly.Block.obtain(workspace,"func_decl_control");
		}
		...
		for (var i = 0; i < variables.length ; i++ ) {
			...
			case "NUMBER":
                                if ( (entry[2] == null) && (Configuration.getParameter("option.noArrays") == "true")) {
                                    block = Blockly.Block.obtain(workspace,"devices_num_noconn");                                    
                                } else {
                                    block = Blockly.Block.obtain(workspace,"devices_num");
                                }
				block.getField_("NAME").setText(entry[0]);
				break;
			case "STRING":
                                if ( (entry[2] == null) && (Configuration.getParameter("option.noArrays") == "true")) {
                                    block = Blockly.Block.obtain(workspace,"devices_string_noconn");                                    
                                } else {
                                    block = Blockly.Block.obtain(workspace,"devices_string");
                                }
				block.getField_("NAME").setText(entry[0]);
				break;
			case "DECIMAL":
                                if ( (entry[2] == null) && (Configuration.getParameter("option.noArrays") == "true")) {
                                    block = Blockly.Block.obtain(workspace,"devices_decimal_noconn");                                    
                                } else {
                                    block = Blockly.Block.obtain(workspace,"devices_decimal");
                                }
				block.getField_("NAME").setText(entry[0]);
				break;
			case "BOOLEAN":
                                if ( (entry[2] == null) && (Configuration.getParameter("option.noArrays") == "true")) {
                                    block = Blockly.Block.obtain(workspace,"devices_boolean_noconn");                                    
                                } else {
                                    block = Blockly.Block.obtain(workspace,"devices_boolean");
                                }
				block.getField_("NAME").setText(entry[0]);
			}
			if ( block ) { 
				...
				connection = block.nextConnection;
 				if ( entry[2] != null ) {
					for (var j = entry[2].length-1; j >= 0 ; j-- ) {
						ndim = Blockly.Block.obtain(workspace,"arr_dimension");
						ndim.initSvg();
						ndim.getField_("LEN").setText(entry[2][j].toString());
						if ( dim != null ) {
