...
TaskOverlay = function () {
    ...
    this.overlay_.setAttribute("draggable","true");
    this.overlay_.onmousedown = TaskOverlay.onmousedown;
    this.overlay_.onmouseup = TaskOverlay.onmouseup;
    this.overlay_.onmousemove = TaskOverlay.onmousemove;
...
TaskOverlay.prototype.clicked = function(event) {
    console.log(event);
}

TaskOverlay.onmousedown = function(event) {
    console.log(event);
    // this.addEventListener("mousemove",TaskOverlay.ondrag,false);
    // event.srcElement.onmousemove = TaskOverlay.ondrag;
    // event.srcElement.onmouseup = TaskOverlay.ondragend;
    TaskOverlay.relX = event.clientX + window.pageXOffset - event.srcElement.offsetLeft;
    TaskOverlay.relY = event.clientY + window.pageYOffset - event.srcElement.offsetTop;
    TaskOverlay.dragging = true;
}

TaskOverlay.onmousemove = function(event) {
    // console.log(event);
    if ( TaskOverlay.dragging ) {
       event.srcElement.style.left = event.clientX + window.pageYOffset - TaskOverlay.relX + 'px';
       event.srcElement.style.top = event.clientY + window.pageYOffset - TaskOverlay.relY + 'px';
    }
}
    
TaskOverlay.onmouseup = function(event) {
    // this.overlay_.removeEventListener('mousemove',this.ondrag,false);
    console.log(event);
    TaskOverlay.dragging = false;
    // var x = event.pageX - this.relX;
    // var y = event.pageY -this.relY;
    // event.srcElement.style.left = event.clientX + window.pageYOffset - TaskOverlay.relX + 'px';
    // event.srcElement.style.top = event.clientY + window.pageYOffset - TaskOverlay.relY + 'px';
    // event.srcElement.removeEventListener('mousemove',TaskOverlay.ondrag,false);    
    // event.srcElement.removeEventListener('mouseup',TaskOverlay.ondragend,false);    
}